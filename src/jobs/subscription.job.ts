...
export async function processSubscriptionRenewals() {
  try {
    ...
    Logger.info(`Found ${subscriptionsDue.length} subscriptions due for renewal`);
    ...
    for (const subscription of subscriptionsDue) {
      try {
        ...
        if (!plan || !plan.isActive) {
          await SubscriptionRepo.updateSubscription({_id:subscription._id}, {
            $set: {
              status:"canceled",
              canceledAt : now
            }
          })
          Logger.info(`Subscription ${subscription._id} canceled due to inactive plan`);
          continue;
        }
    ...
    await SubscriptionRepo.updateSubscription({_id:subscription._id}, {
          ...
          Logger.warn(`Subscription ${subscription._id} renewal failed, attempt #${subscription.failedAttempts}`);
        }
      } catch (error) {
        if (error instanceof Error) {
          Logger.error(`Error processing subscription ${subscription._id}: ${error.message}`);
        } else {
          Logger.error(`Error processing subscription ${subscription._id}: ${String(error)}`);
        }
      }
    }
...
async function processTrialExpirations() {
  ...
  for (const subscription of expiredTrials) {
    try {
      ...
      if (error instanceof Error) {
        Logger.error(`Error processing trial subscription ${subscription._id}: ${error.message}`);
      } else {
        Logger.error(`Error processing trial subscription ${subscription._id}: ${String(error)}`);
      }
    }
  }
}
...
export async function updateSubscriptionStatuses() {
  try {
    ...
    const endOfPeriodCancellations = await SubscriptionRepo.findAllSubscriptions({
      status: 'active',
    ...
    // 2. Handle subscriptions that are past due for too long (e.g., 14 days)
...
// If you want to run this manually for testing
